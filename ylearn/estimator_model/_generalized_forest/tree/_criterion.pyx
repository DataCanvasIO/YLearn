...
cdef class GrfTreeCriterion(CriterionEx):
    ...
    cdef int init_ex(self, const DOUBLE_t[:, ::1] y, const DOUBLE_t[:, ::1] treatment, DOUBLE_t* sample_weight,
        ...
        lwork = max(min(self.d_tr, self.n_node_samples) + 3 * self.n_node_samples + 1, 
        ...
        grad_pvt = <int*> calloc(self.d_tr, sizeof(int))
        
        grad_work = <DOUBLE_t*> malloc(self.d_tr * self.d_tr * sizeof(DOUBLE_t))
        ...
        # TODO: set boundcheck=False
        memset(&self.sum_total[0], 0, self.n_outputs * sizeof(double))
        memset(&self.rho[0], 0, self.n_samples * self.d_tr * sizeof(double))
        ...
        for p in range(start, end):
            ...
            for k_tr in range(self.d_tr):
                ...
                w_tr_ik = w * tr_ik
                tr_node[p + self.n_node_samples * k_tr] = w_tr_ik 
        ...
        for p in range(start, end):
            for k_tr in range(self.d_tr):
                ...
                tr_dif[p + self.n_node_samples * k_tr] = (tr_node[p + self.n_node_samples * k_tr] - 
                                                          self.mean_tr[k_tr])
            ...
            for k in range(self.n_outputs):
                ...
                grad_coef_ik += y_node[p + self.n_node_samples * k] - self.mean_sum[k]
                for k_tr in range(self.d_tr):
                    grad_coef_ik -= tr_dif[p + self.n_node_samples * k_tr] * ls_coef[k_tr + k * self.d_tr]
            
            # # 2: compute grad_coef
            for k_tr in range(self.d_tr):
                tr_dif_ik = tr_node[p + self.n_node_samples * k_tr] - self.mean_tr[k_tr]
                grad_coef[p + self.n_node_samples * k_tr] = tr_dif_ik * grad_coef_ik
                for j_tr in range(self.d_tr):
                    self.grad[k_tr, j_tr] += (tr_dif_ik * tr_dif[p + self.n_node_samples * j_tr] / 
                                              self.n_node_samples)
            ...
            for k_tr in range(self.d_tr):
                for j_tr in range(self.d_tr):
                    self.rho[i] += (f_grad[j_tr + self.d_tr * k_tr] * 
                                    grad_coef[p + self.n_node_samples * j_tr])
    ...
    cdef double node_impurity(self) nogil:
        ...
        return (impurity_left / self.weighted_n_left +
